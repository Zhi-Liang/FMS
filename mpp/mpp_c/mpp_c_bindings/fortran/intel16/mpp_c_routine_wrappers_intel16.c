...
#include "mpp_c_routine_wrappers_intel16.h"
...
/*---------------------------------------------------------------------------*/
/*Send a message.*/
void mpp_c_send_wrap(CFI_cdesc_t *send_data,
                     ...
                     mpp_c_context_t * const * const context)
{
    ...
    {
        mpp_c_send(&(send_data->base_addr),send_len,to_rank,mpp_type,
                     tag,NULL,context);
    }
    else
    {
        mpp_c_send(&(send_data->base_addr),send_len,to_rank,mpp_type,tag,
...
/*---------------------------------------------------------------------------*/
/*Receive a message.*/
void mpp_c_recv_wrap(CFI_cdesc_t *recv_data,
                     ...
                     mpp_c_context_t * const * const context)
{
    if (*request == MPP_C_NULL_REQUEST)
    {
        mpp_c_recv(&(recv_data->base_addr),recv_len,from_rank,mpp_type,
                     tag,NULL,block,context);
    }
    else
    {
        mpp_c_recv(&(recv_data->base_addr),recv_len,from_rank,mpp_type,tag,
...
/*---------------------------------------------------------------------------*/
/*Broadcast a message.*/
void mpp_c_broadcast_wrap(CFI_cdesc_t *bcast_data,
                          ...
                          mpp_c_context_t * const * const context)
{
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_broadcast(&(bcast_data->base_addr),bcast_len,from_rank,
                        mpp_type,NULL,0,context);
    }
    else
    {
        mpp_c_broadcast(&(bcast_data->base_addr),bcast_len,from_rank,
                        mpp_type,rank_list,rank_list_size,context);
    }
...
void mpp_c_sum_wrap(CFI_cdesc_t *sum_data,
                    ...
                    mpp_c_context_t * const * const context)
{
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_sum(&(sum_data->base_addr),sum_len,mpp_type,NULL,0,context);
    }
    else
    {
        mpp_c_sum(&(sum_data->base_addr),sum_len,mpp_type,rank_list,
...
void mpp_c_max_wrap(CFI_cdesc_t *max_data,
                    ...
                    mpp_c_context_t * const * const context)
{
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_max(&(max_data->base_addr),max_len,mpp_type,NULL,0,context);
    }
    else
    {
        mpp_c_max(&(max_data->base_addr),max_len,mpp_type,rank_list,
                  rank_list_size,context);
    }
...
void mpp_c_min_wrap(CFI_cdesc_t *min_data,
                    ...
                    mpp_c_context_t * const * const context)
{
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_min(&(min_data->base_addr),min_len,mpp_type,NULL,0,context);
    }
    else
    {
        mpp_c_min(&(min_data->base_addr),min_len,mpp_type,rank_list,
                  rank_list_size,context);
    }
...
int64_t mpp_c_chksum_wrap(CFI_cdesc_t *chksum_data,
                          size_t const chksum_len,
                          size_t const num_bytes,
                          int8_t const mask_flag,
                          CFI_cdesc_t *mask_val,
                          int32_t const *rank_list,
                          size_t const rank_list_size,
                          mpp_c_context_t * const * const context)
{
    ...
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        if (mask_flag)
        {
            chksum_val = mpp_c_chksum(chksum_data->base_addr,chksum_len,
                                      num_bytes,mask_val->base_addr,NULL,
                                      0,context);
        }
        else
        {
            chksum_val = mpp_c_chksum(chksum_data->base_addr,chksum_len,
                                      num_bytes,NULL,NULL,0,context);
        }
    }
    else
    {
        ...
        {
            chksum_val = mpp_c_chksum(chksum_data->base_addr,chksum_len,
                                      num_bytes,mask_val->base_addr,
                                      rank_list,rank_list_size,context);
        }
        else
        {
            chksum_val = mpp_c_chksum(chksum_data->base_addr,chksum_len,
                                      num_bytes,NULL,rank_list,
                                      rank_list_size,context);
        }
    }
...
void mpp_c_gather_wrap(CFI_cdesc_t *send_data,
                       size_t const send_len,
                       CFI_cdesc_t *recv_data,
                       size_t const recv_len,
                       int32_t const mpp_type,
                       int32_t *rank_list,
                       size_t const rank_list_size,
                       mpp_c_context_t * const * const context)
{
    ...
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_gather(send_data->base_addr,send_len,&(recv_data->base_addr),
                     recv_len,mpp_type,NULL,0,context);
    }
    else
    {
        mpp_c_gather(send_data->base_addr,send_len,&(recv_data->base_addr),
                     recv_len,mpp_type,rank_list,rank_list_size,context);
    }
...
void mpp_c_gatherv_wrap(CFI_cdesc_t *send_data,
                        size_t const send_len,
                        CFI_cdesc_t *recv_data,
                        ...
                        mpp_c_context_t * const * const context)
{
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_gatherv(send_data->base_addr,send_len,&(recv_data->base_addr),
                      recv_data_size,recv_len_array,recv_len_array_size,
                      mpp_type,NULL,0,context);
    }
    else
    {
...
void mpp_c_gather_pelist_wrap(CFI_cdesc_t *gather_data,
                              ...
                              mpp_c_context_t * const * const context)
{
    mpp_c_gather_pelist(&(gather_data->base_addr),gather_data_x_size,
                        gather_data_y_size,gather_data_z_size,mpp_type,
                        &(array_segment->base_addr),gather_spot_x_start_index,
                        gather_spot_x_end_index,gather_spot_y_start_index,
                        gather_spot_y_end_index,gather_spot_z_start_index,
                        gather_spot_z_end_index,rank_list,rank_list_size,
                        rank_list_root_flag,root_gather_spot_x_offset,
                        root_gather_spot_y_offset,root_gather_spot_z_offset,
                        context);
...
/*Perform a generalized scatter of array segments across all ranks on a rank
  list.*/
void mpp_c_scatter_pelist_wrap(CFI_cdesc_t *scatter_data,
                               ...
                               mpp_c_context_t * const * const context)
{
    mpp_c_scatter_pelist(&(scatter_data->base_addr),scatter_data_x_size,
                         scatter_data_y_size,scatter_data_z_size,mpp_type,
                         &(array_segment->base_addr),
...
void mpp_c_alltoall_wrap(CFI_cdesc_t *send_data,
                         size_t const send_len,
                         CFI_cdesc_t *recv_data,
                         size_t const recv_len,
                         ...
                         mpp_c_context_t * const * const context)
{
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_alltoall(send_data->base_addr,send_len,&(recv_data->base_addr),
                       recv_len,size_per_send,size_per_recv,mpp_type,
                       NULL,0,context);
    }
    else
    {
...
void mpp_c_alltoallv_wrap(CFI_cdesc_t *send_data,
                          int32_t *send_len,
                          ...
                          mpp_c_context_t * const * const context)
{
    if (rank_list[0] == MPP_C_NULL_RANK_LIST)
    {
        mpp_c_alltoallv(send_data->base_addr,send_len,send_displace,
                        &(recv_data->base_addr),recv_len,
