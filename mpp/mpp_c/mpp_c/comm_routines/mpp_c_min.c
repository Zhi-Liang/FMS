...
/*---------------------------------------------------------------------------*/
/*Get the minimum value of data across all ranks on a pelist. If a null pelist
               ...
               mpp_c_context_t * const * const context)
{
    ...
    size_t min_data_byte_size = 0;        /*Size of data whose min is found in bytes.*/
    ...
    mpp_c_context_check_init_state(tmp_context,"MPP_C_MIN");

    /*Check array input.*/
    check_array_input((void *)tmp_ptr,min_len,"MPP_C_MIN");

    /*Check pelist input.*/
    check_pelist_input((uint32_t *)rank_list,rank_list_size,"MPP_C_MIN");
    ...
    if (tmp_pelist == NULL)
    {
        error_mesg = "MPP_C_MIN: the inputted pelist does not exist."
                     " You must first create it.";
        mpp_c_error(FATAL,error_mesg,strlen(error_mesg),tmp_ptr);
    }

    /*Find the min if the pelist contains more than just the current
      rank.*/
    if (mpp_c_pelist_get_rank_list_size(tmp_pelist) != 1)
    {
        ...
        /*Get the total size of the data in bytes and allocate the work
          buffer.*/
        min_data_byte_size = get_MPI_data_size_in_bytes(mpi_type_val,min_len);
        ...
        /*Sum the data.*/
        ierr = MPI_Allreduce(tmp_ptr,work_buffer,min_len,
                             mpi_type_val,MPI_MIN,tmp_comm_id);
        check_MPI_error_code(ierr,"MPI_Allreduce","MPP_C_MIN");

        /*Set the inputted data to be equal to the found minimum.*/
        memcpy(tmp_ptr,work_buffer,min_data_byte_size);
